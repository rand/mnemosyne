...
impl MemoryService for MemoryServiceImpl {
    ...
    async fn semantic_search(
        ...
        let results = self.storage.vector_search(
            &req.embedding,
            req.max_results.min(1000) as usize,
            namespace,
        ).await.map_err(|e| Status::from(e))?;
    ...
    async fn graph_traverse(
        ...
        // Parse seed IDs
        let seed_ids: Vec<MemoryId> = req.seed_ids.iter()
        ...
        let memories = self.storage.graph_traverse(
            &seed_ids,
            max_hops,
            None, // namespace filter not used in graph_traverse
        ).await.map_err(|e| Status::from(e))?;
        ...
        let proto_memories: Vec<generated::MemoryNote> = memories
            .into_iter()
    ...
    async fn get_context(
        ...
        if req.memory_ids.is_empty() {
            return Err(Status::invalid_argument("At least one memory ID is required"));
        }

        // Parse memory IDs
        let memory_ids: Vec<MemoryId> = req.memory_ids.iter()
            .map(|id| MemoryId::from_string(id))
            .collect::<Result<Vec<_>, _>>()
            .map_err(|_| Status::invalid_argument("Invalid memory ID format"))?;
        ...
        let linked_memories = if req.include_links {
            ...
            let all_memories = self.storage.graph_traverse(
                &memory_ids,
                max_depth,
                None,
            ).await.map_err(|e| Status::from(e))?;
    ...
    async fn store_memory_stream(
        ...
        tokio::spawn(async move {
            ...
            let _ = tx.send(Ok(StoreMemoryProgress {
            ...
            let memory = InternalMemoryNote {
                ...
                tags,
                context,
                memory_type: memory_type_proto.map(memory_type_from_proto)
                    .unwrap_or(InternalMemoryType::Insight),
