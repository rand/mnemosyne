...
async fn test_project_namespace_isolation() {
    ...
    let mut app1_mem = sample_memory("App1 uses PostgreSQL", MemoryType::ArchitectureDecision, 8);
    app1_mem.namespace = Namespace::Project {
        name: "app1".to_string(),
    };
    app1_mem.keywords = vec!["database".to_string()];

    let mut app2_mem = sample_memory("App2 uses MongoDB", MemoryType::ArchitectureDecision, 8);
...
async fn test_global_search_includes_all_namespaces() {
    ...
    assert!(
        namespaces.contains(&Namespace::Project {
            name: "myapp".to_string()
        }),
...
async fn test_list_memories_by_namespace() {
    ...
    for i in 0..5 {
        ...
        mem.namespace = Namespace::Project {
            name: "myapp".to_string(),
        };
    ...
    for i in 0..3 {
        let mut mem = sample_memory(&format!("Other app memory {}", i), MemoryType::Insight, 5);
        mem.namespace = Namespace::Project {
